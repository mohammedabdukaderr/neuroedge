/**
 * CRC16-CCITT implementation for NeuroEdge firmware.
...
/* Lookup table — computed once, stored in flash via const */
...
uint16_t ne_crc16(const uint8_t *data, size_t len)
{
    uint16_t crc = 0xFFFF;
    while (len--) {
        crc = (uint16_t)((crc << 8) ^ crc16_table[(crc >> 8) ^ *data++]);
    }
    return crc;
}
...
{
    snprintf(buf, 5, "%04X", crc);
}

uint16_t ne_crc16_from_hex(const char *hex)
{
    if (!hex || strlen(hex) < 4) return 0xFFFF;
    uint32_t val = 0;
    for (int i = 0; i < 4; i++) {
        char c = hex[i];
        uint8_t nibble;
